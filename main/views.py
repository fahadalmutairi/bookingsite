...
def filter(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			schedule = Schedule.objects.filter(startsearch & endsearch)

			test = Schedule.objects.get(pk=schedule)
			test2 = test.property_object.booking_set.all()
			for i in test2:
				startle = i.date_start <= start 
				startgt = i.date_start > start
				print startle , startgt
				print start <= end
				print end
				print i.date_start
				print i.date_end
